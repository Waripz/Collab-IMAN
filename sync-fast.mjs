...
const FROM_DATE = '2026-05-01T00:00:00+08:00'; // Fetch from May 1st to fix all PBAKL sales
...
async function run() {
    ...
    try {
      ...
      while (hasNext) {
        ...
        if (pageInfo) {
          ...
          url += `status=any&limit=250&updated_at_min=${FROM_DATE}`;
        }
          ...
          for (const item of order.line_items || []) {
            if (allowedSet.has(item.product_id)) {
              ...
              if (order.taxes_included) {
                const totalTax = (item.tax_lines || []).reduce((s, t) => s + parseFloat(t.price || '0'), 0);
      ...
      console.log(`  Error on store ${store.shop}:`, err.message);
    }
  }
  ...
  console.log(`\n✅ Fast sync complete! ${upserted} recent order items upserted to database.`);
}
